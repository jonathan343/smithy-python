...
final class ClientGenerator implements Runnable {
    ...
    private void generateService(PythonWriter writer) {
        ...
        if (operations.stream()
                .anyMatch(operation -> new OperationInputGenerator(context, operation)
                        .usesDefaultFactory(writer))) {
            writer.addStdlibImport("enum", "Enum");
            writer.addLocallyDefinedSymbol(Symbol.builder().name("_Default").build());
            writer.write("""
                    class _Default(Enum):
                        UNSET = 0
